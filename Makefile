# This Makefile is for the UUID::Pure extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/UUID/Pure.pm]
#     AUTHOR => [q[Yuri A. Kabaenkov <yk@rbmedia.io>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[0] }
#     LICENSE => q[artistic_2]
#     MIN_PERL_VERSION => q[5.006]
#     NAME => q[UUID::Pure]
#     PREREQ_PM => { Digest::SHA=>q[0], Math::BigInt=>q[0], Test::More=>q[0], Time::HiRes=>q[0] }
#     TEST_REQUIRES => { Test::More=>q[0] }
#     VERSION_FROM => q[lib/UUID/Pure.pm]
#     clean => { FILES=>q[UUID-Pure-*] }
...
INST_LIBDIR      = $(INST_LIB)/UUID
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/UUID
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3SECTION) --perm_rw=$(PERM_RW) -u \
	  lib/UUID/Pure.pm $(INST_MAN3DIR)/UUID::Pure.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Generate UUID v7/v8 and get timestammp in ms from it'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Yuri A. Kabaenkov <yk@rbmedia.io>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: UUID-Pure' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Generate UUID v7/v8 and get timestammp in ms from it",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Yuri A. Kabaenkov <yk@rbmedia.io>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "UUID-Pure",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="UUID-Pure" VERSION="0.01">' > UUID-Pure.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Generate UUID v7/v8 and get timestammp in ms from it</ABSTRACT>' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Yuri A. Kabaenkov &lt;yk@rbmedia.io&gt;</AUTHOR>' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Digest::SHA" />' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Math::BigInt" />' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Time::HiRes" />' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="darwin-thread-multi-2level-5.34" />' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> UUID-Pure.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> UUID-Pure.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'example.pl' '$(INST_LIB)/UUID/example.pl' \
	  'lib/UUID/Pure.pm' 'blib/lib/UUID/Pure.pm' 
